...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, isWorkMode, toggleWorkMode }) => {
  ...
  return (
    <>
    <aside className="hidden md:flex flex-col w-64 bg-surface border-r border-bdr h-full relative z-30 transition-colors duration-300">
        <div className="flex items-center space-x-2 p-6 border-b border-bdr h-16 shrink-0">
            <Bot className={`w-8 h-8 ${isWorkMode ? 'text-slate-700 dark:text-slate-200' : 'text-primary'}`} />
            ...
            <span className="text-[10px] bg-surface-hover px-1.5 py-0.5 rounded text-txt-secondary uppercase font-bold">{planLabel}</span>
        </div>
            ...
            return (
              ...
              >
                <div className="flex items-center space-x-3">
                    <Icon className={`w-5 h-5 ${isActive ? (isWorkMode ? 'text-slate-700 dark:text-white' : 'text-primary') : 'text-slate-400'}`} />
        ...
        <div className="flex flex-col shrink-0 bg-surface z-20">
            <div className="px-4 py-2 border-t border-bdr">
                <button
                    onClick={() => onChangeView('settings')}
                    className={`w-full flex items-center space-x-3 px-4 py-2.5 rounded-lg text-sm font-medium transition-colors ${currentView === 'settings' ? 'bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300' : 'text-txt-secondary hover:bg-surface-hover'}`}
                >
                    <SettingsIcon className={`w-5 h-5 ${currentView === 'settings' ? 'text-primary' : 'text-slate-400'}`} />
            ...
            <div className="px-4 pb-2 pt-1">
                <div className="bg-surface-hover rounded-xl p-3 border border-bdr">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-semibold text-txt-secondary uppercase">Modo de Foco</span>
                    </div>
                    <button 
                        onClick={toggleWorkMode}
                        className={`w-full flex items-center justify-between p-2 rounded-lg transition-all duration-300 ${isWorkMode ? 'bg-slate-800 dark:bg-slate-700 text-white shadow-md' : 'bg-surface text-txt-secondary border border-bdr hover:border-indigo-300 dark:hover:border-indigo-700'}`}
                    >
            ...
            {/* User Profile */}
            <div className="p-4 border-t border-bdr bg-surface-subtle">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3 overflow-hidden">
                        ...
                        <div className="min-w-0">
                            <p className="text-sm font-medium text-txt-primary truncate">{user?.displayName || 'Usuário'}</p>
                            <p className="text-xs text-txt-secondary truncate">{isWorkMode ? 'Focado' : 'Online'}</p>
                        </div>
                    </div>

...
export const TaskList: React.FC<TaskListProps> = ({ tasks, onEdit, onDelete, onStatusChange }) => {
  ...
  if (tasks.length === 0) {
      return <div className="p-8 text-center text-slate-500 bg-white rounded-xl border border-slate-200 border-dashed">Nenhuma tarefa encontrada.</div>;
  }
        ...
        <table className="min-w-full divide-y divide-slate-200">
            <thead className="bg-slate-50">
                <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider w-12">Status</th>
                ...
                {tasks.map(task => {
                    ...
                    return (
                        <React.Fragment key={task.id}>
                            <tr className={`hover:bg-slate-50 transition-colors group ${isExpanded ? 'bg-slate-50/50' : ''}`}>
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex items-center space-x-2">
                                            {hasSubtasks && (
                                                <div className="flex items-center text-[10px] text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded-full mt-1 w-fit">
                                                    <ListTree className="w-3 h-3 mr-1" />
                                                    {completedSubs}/{totalSubs}
                                                </div>
                                            )}
                                            {task.description && (
                                                <span className="text-xs text-slate-500 truncate max-w-xs mt-1">{task.description}</span>
                                            )}
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className={`flex items-center text-sm ${overdue ? 'text-red-600 font-medium' : 'text-slate-500'}`}>
                                        {task.dueDate ? (
                                            <>
                                            ...
                                            <span className="text-slate-400 text-xs italic">Sem prazo</span>
                                        )}
                                    </div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-right">
                                    <div className="flex justify-end space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => onEdit(task)} className="text-slate-400 hover:text-indigo-600"><Edit2 className="w-4 h-4" /></button>
                                        <button onClick={() => onDelete(task.id)} className="text-slate-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                                    </div>
                                </td>
                            </tr>
                            ...
                            {isExpanded && hasSubtasks && (
                                <tr>
                                    <td colSpan={6} className="px-0 py-0 border-b border-slate-100 bg-slate-50/20">
